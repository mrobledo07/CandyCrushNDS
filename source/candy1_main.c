/*------------------------------------------------------------------------------
	...
	Programa principal para la prÃƒÂ¡ctica de Computadores: candy-crash para NDS
	(2Ã‚Âº curso de Grado de IngenierÃƒÂ­a InformÃƒÂ¡tica - ETSE - URV)
...
int seed32;						// semilla de nÃƒÂºmeros aleatorios
int level = 0;					// nivel del juego (nivel inicial = 0)
int points;						// contador global de puntos
int movements;					// nÃƒÂºmero de movimientos restantes
int gelees;						// nÃƒÂºmero de gelatinas restantes



/* actualizar_contadores(code): actualiza los contadores que se indican con el
	parÃƒÂ¡metro 'code', que es una combinaciÃƒÂ³n binaria de booleanos, con el
...
int main(void)
{
	...
	int mX, mY, dX, dY;			// variables de detecciÃƒÂ³n de pulsaciones

	seed32 = time(NULL);		// fijar semilla de nÃƒÂºmeros aleatorios
	consoleDemoInit();			// inicializaciÃƒÂ³n de pantalla de texto
	...
	{
		if (initializing)		//////	SECCIÃƒâ€œN DE INICIALIZACIÃƒâ€œN	//////
		{
		...
		else if (falling)		//////	SECCIÃƒâ€œN BAJADA DE ELEMENTOS	//////
		{
			falling = baja_elementos(matrix);	// realiza la siguiente bajada
			retardo(4);
			if (!falling)						// si no estÃƒÂ¡ bajando
			{
				if (hay_secuencia(matrix))		// si hay secuencias
				{
					elimina_secuencias(matrix, mat_mar);	// eliminarlas
		...
		else					//////	SECCIÃƒâ€œN DE JUGADAS	//////
		{
			...
			{	swiWaitForVBlank();				// pantalla tÃƒÂ¡ctil
				scanKeys();
			}
		}
		if (!falling)			//////	SECCIÃƒâ€œN DE DEPURACIÃƒâ€œN	//////
		{
			swiWaitForVBlank();
			scanKeys();
		...
		if (change)				//////	SECCIÃƒâ€œN CAMBIO DE NIVEL	//////
		{
			change = 0;
					...
					|| (movements == 0) || !hay_combinacion(matrix))
			{
				...
				do
				{	swiWaitForVBlank();
					scanKeys();					// esperar pulsaciÃƒÂ³n tecla 'A'
				...
				else
				{
					...
					escribe_matriz(matrix);
					change = 1;					// forzar nueva verificaciÃƒÂ³n
				}								// de combinaciones
				borra_puntuaciones();
			}
			lapse = 0;
		}
		else if (lapse >= 192)	//////	SECCIÃƒâ€œN DE SUGERENCIAS	//////
		{
	...
	return(0);					// nunca retornarÃƒÂ¡ del main
}